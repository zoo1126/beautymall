...
<div class="w" id="f">
	...
	<ul>
		...
		<li><select id="select" style="float:left; margin-left:20px;font-size:20px;margin-top:10px;" onchange="window.location=this.value;">
			<option  >请选择</option>
			<option value="<%=key %>keyword=<%=keyword %>&select=0&man=true" >综合</option>
			<option value="<%=key %>keyword=<%=keyword %>&select=1&man=true">价格升序</option>
			<option value="<%=key %>keyword=<%=keyword %>&select=2&man=true">价格降序</option>
		</select>
		</li>
	</ul>
	</div>
	<div class="showproduct">
		<form >
			<table>
				...
				<td width="60"><p style="font-size:20px;margin-top:30px;">种类</p></td>
