...
response.setCharacterEncoding("UTF-8");
	...
	String all="/beautymall/search_bysort";
	String shouye="/beautymall/shopping/index.jsp";
	int num=0;
	if(id!=null)
	{
		all="/beautymall/search_bysort?id="+id+"&";
		shouye="/beautymall/shopping/index.jsp?id="+id;
	}
	else
	{
		all="/beautymall/search_bysort"+"?";
	}
	if(product!=null)
	{
		num=product.size();
...
<div class="w" id="f">
	...
	<ul>
		...
		<li><select id="select" style="float:left; margin-left:20px;font-size:20px;margin-top:10px;" onchange="window.location=this.value;">
			<option  >请选择</option>
			<option value="<%=all %>sort=<%=sort %>&select=0" >综合</option>
			<option value="<%=all %>sort=<%=sort %>&select=1">价格升序</option>
			<option value="<%=all %>sort=<%=sort %>&select=2">价格降序</option>
		</select>
		</li>
	</ul>
	</div>
