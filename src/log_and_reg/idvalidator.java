...
public class idvalidator  implements Validator{
	...
	public void validate(FacesContext arg0, UIComponent arg1, Object arg2) throws ValidatorException {
		...
		try {
			if(!arg.matches(reg))
			{
				FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "输入有误", "Face号必须由字母、数字或下划线组成");
				arg0.addMessage("userid", message);
				throw new ValidatorException(message);
			}
			else if(dop.isUserexist(arg))
			{
				FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_ERROR, "输入有误", "该Face号已存在");
				arg0.addMessage("userid", message);
				throw new ValidatorException(message);
			}
